...
export function BaseTable<T>({
  ...
  function getFilteredRows(originalArray: T[]) {
    return originalArray.filter((row) =>
      searchFields.some((field) => (row[field] as string).includes(searchValue))
    );
  }
  ...
  const gridColumns: GridColDef[] = columns.map((col) => ({
    ...
    renderCell: (params) => {
      ...
      return col.render ? (
        <Box className="custom-cell">{col.render(value, params.row)}</Box>
      ) : (
        String(value ?? "")
      );
    },
